...
def _create_decoder_arch(config):
    ...
    elif config.name == 'ConvDecoder':
        arch = archs.ConvDecoder(**config)
...
def _create_train_state_discrete(config):
    # Build architecture
    encoder = _create_encoder_arch(config.encoder_arch)
    decoder = _create_decoder_arch(config.decoder_arch)
    arch = DiscreteOperator(encoder, decoder)
    # Initialize parameters
    u = jnp.ones(config.input_dim)
    eps = jnp.ones(config.eps_dim)
    key = random.PRNGKey(config.seed)
    params = arch.init(key, u, eps)
    print(arch.tabulate(key, u, eps))
    # Vectorized functions across a mini-batch
    apply_fn = vmap(arch.apply, in_axes=(None,0,0))
    encode_fn = vmap(lambda params, u, eps: arch.apply(params, u, eps, method=arch._encode), in_axes=(None,0,0))
    decode_fn = vmap(lambda params, beta: arch.apply(params, beta, method=arch._decode), in_axes=(None,0))
    # Optimizaer
    tx = _create_optimizer(config.optim)
    # Create state
    state = TrainState.create(apply_fn=apply_fn,
...
class DiscreteOperator(nn.Module):
    encoder: nn.Module
    decoder: nn.Module

    @nn.compact
    def __call__(self, u, eps):
        beta, _ = self.encoder(u, eps)
        outputs = self.decoder(beta)
        return outputs
...
class DiscreteVAE:
    def __init__(self, config): 
        self.config = config
        self.state = _create_train_state_discrete(config)
        self.beta = config.beta
    ...
    # Computes reconstruction loss across a mini-batch for a single MC sample
    def recon_loss(self, params, u, s, w, eps):
        outputs = self.state.apply_fn(params, u, eps)
        loss = jnp.mean(0.5*w*(s-outputs)**2)
        return loss
    ...
    def loss(self, params, batch):
        ...
        w = weights
        kl_loss = vmap(self.kl_loss, in_axes=(None,None,0))(params, u, eps)
        recon_loss = vmap(self.recon_loss, in_axes=(None,None,None,None,0))(params, u, s, w, eps)
        kl_loss = jnp.mean(kl_loss)
        recon_loss = jnp.mean(recon_loss)
        loss = self.beta*kl_loss + recon_loss
        return loss
