...
def get_config():
    ...
    wandb.project = 'cahn_hilliard_discrete_vae'
    wandb.name = None
    wandb.tag = None

    # Simulation settings
    config.input_dim = (1, 256, 256, 1)
    config.query_dim = 2
    config.eps_dim = 64
    config.beta = 1e-4
    config.seed = 0
    ...
    decoder_arch.name = 'ConvDecoder'
    decoder_arch.input_shape = (8, 8, 128)
    decoder_arch.out_channels = (64, 32, 16, 8, 1)
