...
from layers import Dense, Conv, ConvTranspose, FourierEnc, MultiresEnc, get_voxel_vertices
...
class ConvDecoder(nn.Module):
    ...
    @nn.compact
    def __call__(self, x):
        x = Dense(prod(self.input_shape))(x)
        x = x.reshape(self.input_shape)
        for i in range(len(self.out_channels)-1):
            x = ConvTranspose(self.out_channels[i],  
                              kernel_size=(2,2), 
                              strides=(2,2), 
                              padding="SAME")(x)
            x = self.activation(x)
        x = ConvTranspose(self.out_channels[-1],  
                              kernel_size=(2,2), 
                              strides=(2,2), 
                              padding="SAME")(x)
